...
export class LatencyTracker {
  ...
  public summary(nowMs: number): Record<string, ReturnType<LatencyHistogram["summary"]>> {
    return { feed: this.feed.summary(nowMs), compute: this.compute.summary(nowMs), send: this.send.summary(nowMs), acknowledgment: this.acknowledgment.summary(nowMs), fill: this.fill.summary(nowMs), total: this.total.summary(nowMs) };
  }
}
