...
import { migrateLegacySpotState, LEGACY_SPOT_VERSION, migratePreviousSpotState,
  validatePreviousSpotState, PREVIOUS_SPOT_VERSION, PREVIOUS_SPOT_RUNTIME_SHA256 } from "./migration.js";
...
interface Envelope {
  ...
  runtimeUpgradeEvidence?: { file: string; sha256: string };
}
...
export async function loadSpotPaperEnvelope(file: string, root: string): Promise<Envelope | null> {
  ...
  else if (String(envelope.state.version) === PREVIOUS_SPOT_VERSION) {
    validatePreviousSpotState(envelope.state, envelope.runtimeSourceSha256);
    if (envelope.runtimeUpgradeEvidence !== undefined) throw new Error("SPOT_V2_ALREADY_HAS_UPGRADE_EVIDENCE");
  }
  else validateSpotPaperState(envelope.state);
  const verify = async (proof: { file: string; sha256: string }) => {
    if (!/^(?:cycles\/\d+-\d+(?:-(?:submitted|settled))?|(?:migrations|runtime-upgrades)\/[a-f0-9]{64})\.json$/.test(proof.file)
  ...
  if (envelope.runtimeUpgradeEvidence) {
    const reference = envelope.runtimeUpgradeEvidence;
    if (!/^runtime-upgrades\/[a-f0-9]{64}\.json$/.test(reference.file)) throw new Error("INVALID_SPOT_UPGRADE_PATH");
    const proof = await verify(reference) as unknown as Record<string, unknown>;
    if (proof.version !== "spot-paper-runtime-upgrade-v1" || !Number.isSafeInteger(proof.upgradedAtMs)
      || (proof.upgradedAtMs as number) <= 0 || typeof proof.beforeEnvelopeSha256 !== "string"
      || !/^[a-f0-9]{64}$/.test(proof.beforeEnvelopeSha256)
      || reference.file !== `runtime-upgrades/${proof.beforeEnvelopeSha256}.json`
      || proof.beforeRuntimeSha256 !== PREVIOUS_SPOT_RUNTIME_SHA256
      || proof.afterRuntimeSha256 !== envelope.runtimeSourceSha256
      || proof.accountChanged !== false || proof.ordersChanged !== false || proof.policyChange !== "ENTRY_TIMING_ONLY")
      throw new Error("SPOT_RUNTIME_UPGRADE_EVIDENCE_MISMATCH");
    const backupFile = join(root, `state-v2-${proof.beforeEnvelopeSha256}.json`);
    const backupBytes = await readFile(backupFile);
    if (sha(backupBytes) !== proof.beforeEnvelopeSha256) throw new Error("SPOT_UPGRADE_BACKUP_CHANGED");
    // Recursive loading is bounded: the exact prior schema cannot carry an upgrade reference.
    const backup = await loadSpotPaperEnvelope(backupFile, root);
    if (!backup || backup.runtimeSourceSha256 !== PREVIOUS_SPOT_RUNTIME_SHA256
      || !isDeepStrictEqual(backup.state, proof.before)
      || !isDeepStrictEqual(migratePreviousSpotState(backup.state, backup.runtimeSourceSha256), proof.after)
      || !isDeepStrictEqual(backup.lastEvidence, proof.previousEvidence)
      || !isDeepStrictEqual(backup.migrationEvidence ?? null, proof.previousMigrationEvidence)
      || !isDeepStrictEqual(envelope.migrationEvidence, backup.migrationEvidence)
      || Object.entries(backup.receiptEvidence).some(([id, oldProof]) => !isDeepStrictEqual(envelope.receiptEvidence[id], oldProof)))
      throw new Error("SPOT_RUNTIME_UPGRADE_EVIDENCE_MISMATCH");
  }
  return envelope;
}
...
/** Caller holds the runner lock; only the known settled v2 journal can cross this policy boundary. */
export async function upgradeSpotPaperEnvelope(envelope: Envelope, root: string, sourceSha256: string): Promise<Envelope> {
  if (!/^[a-f0-9]{64}$/.test(sourceSha256) || sourceSha256 === PREVIOUS_SPOT_RUNTIME_SHA256)
    throw new Error("INVALID_SPOT_UPGRADE_SOURCE_HASH");
  if (envelope.runtimeUpgradeEvidence !== undefined) throw new Error("SPOT_V2_ALREADY_HAS_UPGRADE_EVIDENCE");
  const state = migratePreviousSpotState(envelope.state, envelope.runtimeSourceSha256);
  const file = join(root, "state.json"), oldBytes = await readFile(file);
  if (!isDeepStrictEqual(JSON.parse(oldBytes.toString("utf8")), envelope)
    || !isDeepStrictEqual(await loadSpotPaperEnvelope(file, root), envelope))
    throw new Error("SPOT_STATE_CHANGED_DURING_UPGRADE");
  const oldSha = sha(oldBytes), proofFile = `runtime-upgrades/${oldSha}.json`;
  await mkdir(join(root, "runtime-upgrades"), { recursive: true });
  const backup = join(root, `state-v2-${oldSha}.json`);
  try { await durableFile(backup, oldBytes.toString("utf8"), true); }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST" || sha(await readFile(backup)) !== oldSha) throw error; }
  let proofBytes: string;
  try { proofBytes = await readFile(join(root, proofFile), "utf8"); }
    ...
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    proofBytes = JSON.stringify({ version: "spot-paper-runtime-upgrade-v1", upgradedAtMs: Date.now(),
      beforeEnvelopeSha256: oldSha, beforeRuntimeSha256: envelope.runtimeSourceSha256, afterRuntimeSha256: sourceSha256,
      before: envelope.state, after: state, previousEvidence: envelope.lastEvidence,
      previousMigrationEvidence: envelope.migrationEvidence ?? null,
      accountChanged: false, ordersChanged: false, policyChange: "ENTRY_TIMING_ONLY" }) + "\n";
    await durableFile(join(root, proofFile), proofBytes, true);
  }
  const proof = JSON.parse(proofBytes) as Record<string, unknown>;
  if (proof.version !== "spot-paper-runtime-upgrade-v1" || !Number.isSafeInteger(proof.upgradedAtMs)
    || (proof.upgradedAtMs as number) <= 0 || proof.beforeEnvelopeSha256 !== oldSha
    || proof.beforeRuntimeSha256 !== envelope.runtimeSourceSha256 || proof.afterRuntimeSha256 !== sourceSha256
    || !isDeepStrictEqual(proof.before, envelope.state) || !isDeepStrictEqual(proof.after, state)
    || !isDeepStrictEqual(proof.previousEvidence, envelope.lastEvidence)
    || !isDeepStrictEqual(proof.previousMigrationEvidence, envelope.migrationEvidence ?? null)
    || proof.accountChanged !== false || proof.ordersChanged !== false || proof.policyChange !== "ENTRY_TIMING_ONLY")
    throw new Error("SPOT_RUNTIME_UPGRADE_EVIDENCE_MISMATCH");
  const runtimeUpgradeEvidence = { file: proofFile, sha256: sha(proofBytes) };
  const upgraded: Envelope = { ...envelope, state, runtimeSourceSha256: sourceSha256,
    lastEvidence: runtimeUpgradeEvidence, runtimeUpgradeEvidence };
  await durableFile(file, JSON.stringify(upgraded) + "\n", false);
  return upgraded;
}
...
export async function runSpotPaper(args = process.argv.slice(2)) {
  ...
  try {
    ...
    if (String(envelope.state.version) === LEGACY_SPOT_VERSION)
      ...
      envelope = await upgradeSpotPaperEnvelope(envelope, root, sourceSha256);
    ...
    const status = () => ({ system: "BTC spot weekly trend", mode: "RESEARCH_PAPER", liveTradingEnabled: false,
      ...
      lastError, lastSuccessMs, evidence: { ...evidence.summary, scope: "PARENT_WEEKLY_STRATEGY",
        revisedTimingValidated: false, revisedTimingStatus: "PROSPECTIVE_PAPER_EVALUATION" }, state: envelope!.state });
    if (mode === "--serve") {
