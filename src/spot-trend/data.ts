...
export interface SpotWeekDataset {
  ...
  coverage: {
    ...
    gaps: Array<{ fromMs: number; toMs: number; weeks: number }>;
  };
  limitations: string[];
}
...
function object(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("INVALID_SPOT_WEEK_DOCUMENT");
  return value as Record<string, unknown>;
}

function sourceRows(document: unknown): unknown[] {
  const value = object(document);
  if (Object.keys(value).some(key => key !== "error" && key !== "result")
    || !Array.isArray(value.error) || value.error.length !== 0) throw new Error("SPOT_WEEK_API_ERROR");
  const result = object(value.result);
  if (Object.keys(result).some(key => key !== "XXBTZUSD" && key !== "last")
    || !Number.isSafeInteger(result.last) || (result.last as number) < 0
...
export function parseSpotWeeks(document: unknown, asOfMs: number): SpotWeek[] {
  if (!Number.isSafeInteger(asOfMs) || asOfMs < 0) throw new Error("INVALID_SPOT_WEEK_AS_OF");
  const bars: SpotWeek[] = [];
  let priorSourceMs = -1;
  for (const raw of sourceRows(document)) {
    if (!Array.isArray(raw) || raw.length !== 8 || !Number.isSafeInteger(raw[0]) || (raw[0] as number) < 0)
    ...
    if (availableAtMs > asOfMs) continue;
    ...
    if (!numeric.every(Number.isFinite) || Math.min(open, high, low, close, vwap) <= 0 || volume < 0
      ...
      || !Number.isSafeInteger(trades) || (trades as number) < 0 || ((trades as number) === 0) !== (volume === 0))
      throw new Error("INVALID_SPOT_WEEK_OHLCVT");
    if (bars.length && bars.at(-1)!.endMs !== openMs) throw new Error("MISSING_COMPLETED_SPOT_WEEK");
    bars.push({ openMs, endMs, availableAtMs, open, high, low, close, volume, trades: trades as number });
  }
  return bars;
}

function reconstruct(source: Buffer, retrievedAtMs: number): SpotWeekDataset {
  if (!source.length || source.length > SPOT_WEEK_DATA_SPEC.maximumResponseBytes) throw new Error("SPOT_WEEK_SOURCE_LIMIT");
  const document: unknown = JSON.parse(source.toString("utf8"));
  const bars = parseSpotWeeks(document, retrievedAtMs);
  if (!bars.length) throw new Error("NO_COMPLETED_SPOT_WEEKS");
...
async function boundedResponse(response: Response): Promise<Buffer> {
  if (!response.ok || !response.body) throw new Error(`SPOT_WEEK_HTTP_${response.status}`);
  ...
  try {
    for (;;) {
      ...
      if (bytes > SPOT_WEEK_DATA_SPEC.maximumResponseBytes) throw new Error("SPOT_WEEK_RESPONSE_LIMIT");
...
export async function downloadSpotWeeks(root: string, dependencies: { fetcher?: typeof fetch; nowMs?: number } = {}): Promise<SpotWeekDataset> {
  const registeredAtMs = dependencies.nowMs ?? Date.now();
  if (!root || !Number.isSafeInteger(registeredAtMs) || registeredAtMs < 0) throw new Error("INVALID_SPOT_WEEK_DOWNLOAD");
  ...
  const source = await boundedResponse(await (dependencies.fetcher ?? fetch)(SPOT_WEEK_DATA_SPEC.sourceUrl,
...
async function boundedFile(path: string): Promise<Buffer> {
  ...
  if (!info.isFile() || info.size <= 0 || info.size > SPOT_WEEK_DATA_SPEC.maximumResponseBytes) throw new Error("SPOT_WEEK_FILE_LIMIT");
  const bytes = await readFile(path);
  if (bytes.length !== info.size || bytes.length > SPOT_WEEK_DATA_SPEC.maximumResponseBytes) throw new Error("SPOT_WEEK_FILE_CHANGED");
  return bytes;
}

/** Reconstruct every retained bar from the hash-bound public source, not cached features. */
export async function loadSpotWeeks(root: string): Promise<SpotWeekDataset> {
  ...
  if (Object.keys(manifest).length !== expectedKeys.length || expectedKeys.some(key => !(key in manifest))
    ...
    || JSON.stringify(manifest.spec) !== JSON.stringify(SPOT_WEEK_DATA_SPEC)) throw new Error("INVALID_SPOT_WEEK_MANIFEST");
  const source = await boundedFile(join(root, "source.json"));
  if (source.length !== manifest.sourceBytes || sha256(source) !== manifest.sourceSha256) throw new Error("SPOT_WEEK_SOURCE_HASH_MISMATCH");
  const cached = await boundedFile(join(root, "dataset.json"));
  if (cached.length !== manifest.datasetBytes || sha256(cached) !== manifest.datasetSha256) throw new Error("SPOT_WEEK_DATASET_HASH_MISMATCH");
  const reconstructed = reconstruct(source, manifest.retrievedAtMs as number);
  if (JSON.stringify(JSON.parse(cached.toString("utf8"))) !== JSON.stringify(reconstructed)) throw new Error("SPOT_WEEK_SOURCE_DATASET_MISMATCH");
  const registration = object(JSON.parse((await boundedFile(join(root, "registration.json"))).toString("utf8")));
  if (!Number.isSafeInteger(registration.registeredAtMs) || (registration.registeredAtMs as number) < 0
    || (registration.registeredAtMs as number) > reconstructed.retrievedAtMs
    ...
    || registration.activationAllowed !== false) throw new Error("INVALID_SPOT_WEEK_REGISTRATION");
  return reconstructed;
}

export async function main(args = process.argv.slice(2)): Promise<void> {
  if (args.length !== 1 || !args[0]) throw new Error("Usage: npx tsx src/spot-trend/data.ts NEW_OUTPUT_DIRECTORY");
