...
export class DeterministicEdgeResolver {
  ...
  private analytic(input: EdgeResolutionInput): EdgeEstimate {
    ...
    const kinematicQuality = (
      positiveRatio(side * features.velocityZ, this.cfg.velocityScale)
      + positiveRatio(side * features.microEdgeBps, this.cfg.microEdgeScaleBps)
      + positiveRatio(directionalBreakoutBps, this.cfg.breakoutScaleBps)
    ) / 3;
    const quality = clamp(
      .30 * scoreQuality + .20 * input.persistence + .15 * features.efficiency
      + .20 * flowQuality + .15 * kinematicQuality,
      0, 1,
    );
...
function validateAnalyticEdgeConfig(cfg: AnalyticEdgeConfig): void {
  const positive = [cfg.economicHorizonMs, cfg.qiKScale, cfg.ofiScale, cfg.tfiScale, cfg.velocityScale,
    cfg.microEdgeScaleBps, cfg.breakoutScaleBps, cfg.maximumGrossBps];
