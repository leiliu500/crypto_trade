...
export interface ProfitValidationIdentity {
  strategyVersion: string; candidatePolicy: string; validationSpecSha256: string;
}
function validateRun(value: unknown, benchmark: boolean, identity: ProfitValidationIdentity): ProfitRunValidation {
  ...
  fail(r.version !== identity.strategyVersion || r.evidenceKind !== PROFIT_VALIDATION_SPEC.historicalEvidenceKind
  ...
  fail(benchmark ? !PROFIT_VALIDATION_SPEC.benchmarkPolicies.some(p => p === policy) || r.benchmarkOnly !== true
    : policy !== identity.candidatePolicy || r.benchmarkOnly !== false, "POLICY_IDENTITY_INVALID");
...
export function validateProfitStudy(input: ProfitValidationInput) {
  return validateProfitStudyIdentity(input, { strategyVersion: S.version, candidatePolicy: "weekly-forecast",
    validationSpecSha256: PROFIT_VALIDATION_SPEC_SHA256 });
}

/** Reuses the exact same economic thresholds for a separately registered
 * candidate identity. Source/protocol provenance still needs independent
 * verification; this function never grants runtime order authorization. */
export function validateProfitStudyIdentity(input: ProfitValidationInput, identity: ProfitValidationIdentity) {
  ...
  const validEnvelope = object(envelope) && Array.isArray(envelope.runs) && Array.isArray(envelope.benchmarks)
    && Boolean(identity && typeof identity.strategyVersion === "string" && identity.strategyVersion.length
      && typeof identity.candidatePolicy === "string" && identity.candidatePolicy.length
      && /^[a-f0-9]{64}$/.test(identity.validationSpecSha256));
  ...
  const checks = runs.map(r => validateRun(r, false, identity));
  const benchmarkChecks = benchmarks.map(r => validateRun(r, true, identity));
  ...
  const runSetComplete = complete(checks, expected([identity?.candidatePolicy ?? "INVALID_IDENTITY"]));
  ...
  const benchmarkComparisons = PROFIT_VALIDATION_SPEC.fundingAssumptions.map(fundingAssumption => {
    const periods = PROFIT_STUDY_WINDOWS.map(w => {
      const candidate = checks.filter(r => r.key === `${w.id}:${identity?.candidatePolicy}:base:${fundingAssumption}`);
  ...
  return { validationSpecSha256: identity?.validationSpecSha256 ?? null,
