...
import { replayProfitUtility } from "./utility-replay.js";
import { PROFIT_UTILITY_VALIDATION_SPEC, validateProfitUtilityStudy } from "./utility-validation.js";
...
const SOURCES = ["src/profit/spec.ts", "src/profit/model.ts", "src/profit/replay.ts", "src/profit/study.ts",
  ...
  "src/profit/utility.ts", "src/profit/utility-replay.ts", "src/profit/utility-validation.ts",
...
export async function runProfitStudy(dataDirectories: readonly string[], outputDirectory: string,
  variant: "confidence" | "utility" = "confidence") {
  if (!["confidence", "utility"].includes(variant)) throw new Error("PROFIT_STUDY_INVALID_VARIANT");
  const utility = variant === "utility";
  const strategy = utility ? PROFIT_UTILITY_SPEC : PROFIT_SPEC;
  const validationSpec = utility ? PROFIT_UTILITY_VALIDATION_SPEC : PROFIT_VALIDATION_SPEC;
  ...
  const protocol = { registeredAtMs, registeredAt: new Date(registeredAtMs).toISOString(),
    strategy, modelSpecification: PROFIT_SPEC, variant, study: D, validation: validationSpec, executionAssumptions: PROFIT_REPLAY_ASSUMPTIONS,
  ...
  for (const window of D.windows) for (const scenario of D.scenarios) for (const fundingAssumption of D.fundingAssumptions) {
    for (const policy of [utility ? "weekly-mean-variance" : "weekly-forecast", ...D.benchmarkPolicies] as const) {
      const input = { ...data, forecasts: prepared.forecasts, fits: prepared.fits,
        startMs: window.startMs, endMs: window.endMs, scenario, fundingAssumption };
      const run = utility ? replayProfitUtility({ ...input, policy: policy as "weekly-mean-variance" | typeof D.benchmarkPolicies[number] })
        : replayProfit({ ...input, policy: policy as "weekly-forecast" | typeof D.benchmarkPolicies[number] });
      (policy.startsWith("weekly-") ? runs : benchmarks).push(run);
  ...
  const validation = utility ? validateProfitUtilityStudy({ runs, benchmarks }) : validateProfitStudy({ runs, benchmarks });
  ...
  const report = { generatedAt: new Date().toISOString(), sourceSha256, dataSha256: data.dataSha256,
    strategyVersion: strategy.version, variant, validation, modelCount: prepared.fits.length,
  ...
  await writeFile(join(out, "report.md"), [
    `Weekly inventory candidate ${strategy.version}: historical economic results. No future profitability is guaranteed.`, "",
    `Historical eligibility: ${validation.historicalEligible ? "passed" : "failed"}. Runtime activation: false.`, "",
...
async function main() {
  ...
  let out: string | undefined, variant: "confidence" | "utility" = "confidence";
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--help") {
      console.log("Usage: npm run research:profit -- --out-dir NEW_DIRECTORY [--variant confidence|utility] [--data-dir DIRECTORY ...]\nUses frozen 2024 and 2025 H1 windows; keeps reserved 2026 data excluded. Never overwrites an existing protocol."); return;
    }
    ...
    else if (key === "--variant" && ["confidence", "utility"].includes(value)) variant = value as typeof variant;
  ...
  const report = await runProfitStudy(dirs, out, variant);
  console.log(JSON.stringify({ historicalEligible: report.validation.historicalEligible,
    paperPilotAllowed: report.validation.paperPilotAllowed, fullValidationPassed: report.validation.fullValidationPassed,
    reasons: report.validation.reasons, modelCount: report.modelCount, forecastCount: report.forecastCount }, null, 2));
}
