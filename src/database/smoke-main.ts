...
await store.start({ mode: "replay", paper: false, strategyVersion: cfg.strategyVersion, modelVersion: cfg.modelVersion,
...
const pool = new Pool({ connectionString: cfg.databaseUrl, max: 1, connectionTimeoutMillis: 5_000,
  application_name: "crypto-trade-smoke-verifier" });
try {
  const result = await pool.query<{ run_id: string; event_count: string }>(`SELECT r.id::text AS run_id,
      (SELECT count(*)::text FROM system_events e WHERE e.run_id=r.id) AS event_count
    FROM engine_runs r WHERE r.metadata->>'purpose' = 'database-smoke-test'
