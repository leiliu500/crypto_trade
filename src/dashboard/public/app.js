...
function renderLivePnl(position){
  if(!position)return "";
  const history=(position.pnlHistory||[]).slice().reverse().map(point=>`<div class="pnl-change-row"><time>${time(point.atMs)}</time><span>${money(point.currentPx,point.currentPx>1000?2:4)}</span><strong class="${pnlClass(point.unrealizedPnl)}">${signedMoney(point.unrealizedPnl)}</strong><em class="${pnlClass(point.changePnl)}">${point.changePnl==null?"initial":signedMoney(point.changePnl)}</em></div>`).join("");
  const stateLabel=position.active?"OPEN":"CLOSED";
  const title=position.active?"Live position P&amp;L":"Closed position P&amp;L history";
  ...
  return `<section class="order-live-pnl ${position.active?"active":"closed"}" data-testid="order-live-pnl" aria-live="polite"><div class="live-pnl-head"><div><span>${title}</span><strong class="${pnlClass(position.unrealizedPnl)}">${signedMoney(position.unrealizedPnl)}</strong></div><div><span class="pnl-position-state">${stateLabel}</span><strong class="${pnlClass(position.unrealizedPnlBps)}">${signed(position.unrealizedPnlBps," bp")}</strong></div></div><div class="live-pnl-meta"><span>Entry ${money(position.entryPx,position.entryPx>1000?2:4)}</span><span>Last mark ${money(position.currentPx,position.currentPx>1000?2:4)}</span><span>${ageLabel} ${duration(position.ageMs)}</span></div><div class="pnl-history"><div class="pnl-history-title"><span>All P&amp;L changes</span><span>mark / total / change</span></div>${history||"<div class='pnl-history-empty'>Waiting for the next price change…</div>"}</div><div class="live-pnl-action"><b>${esc(position.latestAction)}</b><span title="${esc(context)}">${esc(context)}</span></div></section>`;
}
