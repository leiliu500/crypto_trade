...
function render(s){
  ...
  renderLiveness(s.liveness||[]);syncSymbols(s.markets||[]);renderMarkets(filtered(s.markets||[]));renderPositions(filtered(s.positions||[]));renderOrders(filtered(s.orders||[]));renderEvents(s.events||[]);
...
function renderPositions(items){el("position-count").textContent=`${items.length} active`;const grid=el("positions-grid");if(!items.length){grid.className="positions-grid empty-grid";grid.innerHTML="<p>No active positions. Exit intelligence will appear here.</p>";return;}grid.className="positions-grid";grid.innerHTML=items.map(p=>{
  const progress=Math.max(4,Math.min(96,33+(Number(p.currentPx)-p.entryPx)/(Math.max(p.initialRiskPx,.000001))*22));const reason=p.latestReason||"Evidence remains inside hold envelope";
  return `<article class="position-card"><div class="position-head"><div><span class="symbol">${esc(p.symbol)}</span><span class="side-label ${p.side<0?"sell":""}">${p.side>0?"LONG":"SHORT"} · ${num(p.qty,6)}</span><div class="venue">OPEN ${duration(p.ageMs)} · ENTRY ${money(p.entryPx,p.entryPx>1000?2:4)}</div></div><span class="phase">${esc(p.phase)}</span></div><div class="position-pnl"><div><span>Unrealized P&amp;L</span><strong class="${pnlClass(p.unrealizedPnl)}">${money(p.unrealizedPnl)}</strong></div><strong class="${pnlClass(p.unrealizedPnlBps)}">${signed(p.unrealizedPnlBps," bp")}</strong></div><div class="exit-track"><div class="exit-track-label"><span>Hard risk −1R</span><span>Current</span><span>MFE +${num(p.mfePx,2)}</span></div><div class="track"><i class="risk-zone"></i><i class="reward-zone"></i><i class="track-marker" style="left:${progress}%"></i></div></div><div class="position-metrics"><div><span>Live exit</span><strong>${money(p.currentPx,p.entryPx>1000?2:4)}</strong></div><div><span>Profit floor</span><strong>${money(p.stopPx,p.entryPx>1000?2:4)}</strong></div><div><span>Reversal</span><strong>${p.reversalProbability==null?"—":num(p.reversalProbability*100,1)+"%"}</strong></div></div><div class="decision-strip"><div><small>Latest action</small><b>${esc(p.latestAction)}</b></div><small>${esc(reason)}</small></div></article>`;}).join("");}
function renderOrders(items){items=items.filter(order=>state.orderFilter==="all"||(state.orderFilter==="open"?!order.terminal:order.terminal));const grid=el("orders-grid");if(!items.length){grid.className="orders-grid empty-grid";grid.innerHTML="<p>No orders match this view.</p>";return;}grid.className="orders-grid";grid.innerHTML=items.map(o=>{
