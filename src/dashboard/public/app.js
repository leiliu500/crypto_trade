...
function policyFamilyName(family){return {CONTINUATION:"Trend",EARLY_BREAKOUT:"Breakout",PULLBACK_RECOVERY:"Recovery"}[family]||String(family);}
function renderPolicyMarket(m){
  ...
  const rows=(p.families||[]).map(f=>`<div class="policy-family"><div><b>${esc(policyFamilyName(f.family))}</b><small>${(f.horizonsMs||[]).map(h=>`${num(h/60000,0)}m`).join(" / ")}</small></div><span class="policy-signal ${f.longSignal?"matched":""}">LONG ${f.longSignal?"MATCH":"—"}</span><span class="policy-signal ${f.shortSignal?"matched":""}">SHORT ${f.shortSignal?"MATCH":"—"}</span></div>`).join("");
