...
async function bootstrap(){
  ...
  connect();setInterval(()=>{el("clock").textContent=new Date().toLocaleTimeString([],{hour12:false});if(state.snapshot){el("last-update").textContent=`Updated ${relative(state.snapshot.generatedAtMs)}`;refreshCrossAssetPanels();}},1000);
}
function connect(){
...
function render(s){
  ...
  el("market-subtitle").textContent=hasJointModel?`BTC / ETH JOINT MODEL · ${s.crossAssetPaperEntriesEnabled&&s.mode==="paper"?"PAPER ORDERS ENABLED":"RESEARCH"}`:s.policyEngineEnabled&&!s.paperEntryExercise?"EXECUTABLE POLICIES · LIVE SIGNALS & PAPER RESEARCH":"KRAKEN FUTURES MARKET DATA · LEGACY RULES";
...
function renderCrossAssetModel(m,nowMs=Math.max(state.snapshot?.generatedAtMs||0,Date.now())){
  ...
  if(knownVersion&&labels!==null){
    ...
    }else if(!f){status="WAITING FOR FORECAST";detail="Training minimum met. Waiting for a forecast from fresh BTC and ETH quotes.";}
    ...
    else if(!current){status="FORECAST EXPIRED";detail="The last forecast is for reference only. Entry checks require a forecast no more than one second old.";}
  ...
  const entryGate=enabled?(state.snapshot.entriesAllowed!==true?"Engine health or risk gates currently block new entries.":
    ({POSITION_OPEN:"An existing position blocks another entry.",ORDER_PENDING:"An order is pending; another entry is blocked.",
      ...
      ||"Qualifying forecasts can enter the paper order checks.")):
  ...
  const forecastTime=forecastAge===null?"No forecast yet":forecastAge<0?"Forecast timestamp is ahead of the current clock":`${time(f.atMs)} · ${duration(forecastAge)} ago${current?" · current":" · not current"}`;
  ...
  return `<section class="cross-asset-panel ${tone}" data-testid="cross-asset-panel" data-symbol="${esc(m.symbol)}" aria-label="${esc(m.symbol)} joint model"><div class="joint-heading"><div><h3>BTC/ETH joint model</h3><small>${esc(learning.version||"Version unavailable")}</small></div><span class="joint-submission ${enabled?"enabled":"disabled"}">PAPER ORDERS ${enabled?"ENABLED":"DISABLED"}</span></div><p class="joint-entry-gate">${esc(entryGate)}</p><div class="joint-training"><div><span>Completed 15m training intervals</span><strong>${count}</strong></div>${progress}${trainingSource}<small>${num(learning.historySamples,0)} recent minute samples · ${num(learning.invalidLabelPairs,0)} discarded intervals</small></div><div class="joint-status"><b>${status}</b><p>${esc(detail)}</p></div>${metrics}<div class="joint-forecast-time">${esc(forecastTime)}</div>${reason}<small class="joint-limit">Paper cap ${money(p.maximumNotional,0)} · shared 30m entry cooldown · profitability unproven</small></section>`;
}
...
function renderPolicyMarket(m){
  ...
  const mode={PAPER_RESEARCH:p.research?.crossAsset?"PAPER RESEARCH · JOINT MODEL + POLICIES":"PAPER RESEARCH · UNSCORED",CALIBRATED_PAPER:"CALIBRATED PAPER ONLY",SHADOW:"SHADOW · NO ORDERS",RECORD:"RECORD · NO ORDERS"}[p.mode]||p.mode;
