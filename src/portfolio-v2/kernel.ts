import { PORTFOLIO_SPEC, PORTFOLIO_SYMBOLS, PORTFOLIO_VERSION,
  type AssetRules, type Pair, type PortfolioFill, type PortfolioOrder, type PortfolioPlan as BasePortfolioPlan,
  type PortfolioPlannerInput as BasePortfolioPlannerInput, type PortfolioPosition, type PortfolioState, type PortfolioSymbol } from "../portfolio/types.js";

// Deliberate v1 ledger/allocator fork: sealed src/portfolio/kernel.ts remains
// byte-identical. This v2 adds a per-plan execution cap, while retaining the
// fixed $12 objective denominator and the existing receipt/accounting schema.
export interface PortfolioPlannerInput extends BasePortfolioPlannerInput { maximumGrossNotionalUsd?: number }
export interface PortfolioPlan extends BasePortfolioPlan { maximumGrossNotionalUsd: number }
...
export function planPortfolioAdjustment(input: PortfolioPlannerInput): PortfolioPlan {
  ...
  const cap = input.maximumGrossNotionalUsd === undefined ? CAP : input.maximumGrossNotionalUsd;
  if (!finite(cap) || cap < 0 || cap > CAP) throw new Error("PORTFOLIO_INVALID_DYNAMIC_CAP");
  assertState(state);
  ...
  const desired = pair(s => input.forceFlat || expired || cap === 0 ? 0 : target?.targetUsd?.[s] ?? 0);
  ...
  const make = (status: PortfolioPlan["status"], reason: string, orders: PortfolioOrder[] = []): PortfolioPlan => {
    ...
    return { atMs, targetDecisionMs: time(target?.decisionMs) ? target.decisionMs : 0, status, reason,
      ...
      maximumGrossNotionalUsd: cap, unusedCapacityUsd: Math.max(0, cap - grossNotionalUsd), estimatedAdjustmentCostUsd, orders };
  };
...
export function reservePortfolioOrders(state: PortfolioState, plan: PortfolioPlan): PortfolioState {
  assertState(state);
  if (!plan || !finite(plan.maximumGrossNotionalUsd) || plan.maximumGrossNotionalUsd < 0 || plan.maximumGrossNotionalUsd > CAP
    || !time(plan.atMs) || !time(plan.targetDecisionMs) || plan.targetDecisionMs > plan.atMs
  ...
  if (PORTFOLIO_SYMBOLS.some(s => !qtyNear(after[s], plan.executableQty[s])) || !near(gross, plan.grossNotionalUsd)
    || !near(Math.max(0, plan.maximumGrossNotionalUsd - gross), plan.unusedCapacityUsd)
    || plan.status === "INCREASE" && gross > plan.maximumGrossNotionalUsd
