...
export function spotTrendSignal(bars, nowMs, previous) {
    if (!Number.isSafeInteger(nowMs) || nowMs < 0 || !["long", "cash"].includes(previous))
        throw new Error("INVALID_SPOT_SIGNAL_ARGUMENT");
    ...
    for (let i = 0; i < window.length; i++) {
        ...
        if (![b.close, b.openMs, b.endMs, b.availableAtMs].every(Number.isFinite) || b.close <= 0
            ...
            || i > 0 && b.openMs !== window[i - 1].endMs)
            throw new Error("INVALID_SPOT_SIGNAL_HISTORY");
    }
    ...
    if (window.length < S.movingAverageWeeks)
        return { ...base, state: "cash", reason: "WARMUP", movingAverage: null };
    const movingAverage = window.reduce((sum, b) => sum + b.close, 0) / window.length;
    if (last.close <= movingAverage)
        return { ...base, state: "cash", reason: "TREND_EXIT", movingAverage };
    if (last.close > movingAverage * (1 + S.entryBufferFraction))
        return { ...base, state: "long", reason: "TREND_ENTER", movingAverage };
    return { ...base, state: previous, reason: "HOLD_BAND", movingAverage };
}
export function reconstructSpotTrend(bars, nowMs) {
    let signal = spotTrendSignal([], nowMs, "cash");
    for (const bar of bars) {
        if (bar.availableAtMs > nowMs)
            break;
