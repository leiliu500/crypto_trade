...
const fail = (reason) => { throw new Error(`SPOT_PAPER_EVIDENCE_${reason}`); };
const object = (value) => value && typeof value === "object" && !Array.isArray(value)
    ? value : fail("INVALID_DOCUMENT");
const number = (value) => typeof value === "number" && Number.isFinite(value) ? value : fail("INVALID_NUMBER");
const integer = (value) => Number.isSafeInteger(value) && value >= 0 ? value : fail("INVALID_COUNT");
const array = (value) => Array.isArray(value) ? value : fail("INVALID_ARRAY");
const near = (a, b) => Math.abs(a - b) <= 1e-7;
const same = (a, b, reason) => { if (!isDeepStrictEqual(a, b))
    fail(reason); };
const sha = (value) => typeof value === "string" && /^[a-f0-9]{64}$/.test(value) ? value : fail("INVALID_HASH");
function safeRelative(value) {
    if (typeof value !== "string" || !/^[A-Za-z0-9_-][A-Za-z0-9_.-]*(?:\/[A-Za-z0-9_-][A-Za-z0-9_.-]*)*$/.test(value)
        || value.split("/").some(part => part === "." || part === "..")
        || !(value.startsWith("reports/") || SPOT_STUDY_SOURCES.includes(value)))
        fail("UNSAFE_PATH");
    return value;
}
/** An audited historical nomination can permit research paper only, never assert profit. */
export async function loadSpotPaperEvidence(studyDirectory, auditFile, repoRoot = process.cwd()) {
    const root = await realpath(resolve(repoRoot));
    const relativeInput = (value) => safeRelative(relative(root, resolve(root, value)).split(sep).join("/"));
    const studyPath = relativeInput(studyDirectory), auditPath = relativeInput(auditFile);
    if (!studyPath.startsWith("reports/") || !auditPath.startsWith("reports/"))
    ...
    const read = async (path) => {
        ...
        if (!info.isFile() || info.size <= 0 || info.size > 16 * 1024 * 1024)
            fail("FILE_LIMIT");
        const bytes = await readFile(target);
        totalBytes += bytes.length;
        if (bytes.length !== info.size || totalBytes > 64 * 1024 * 1024)
            fail("FILE_CHANGED_OR_TOTAL_LIMIT");
        cache.set(path, bytes);
        return bytes;
    };
    const auditBytes = await read(auditPath), audit = object(JSON.parse(auditBytes.toString("utf8")));
    if (audit.passed !== true || integer(audit.checks) < 1)
        fail("AUDIT_NOT_PASSED");
    for (const key of ["failures", "failedChecks", "errors"])
        if (key in audit) {
            const value = audit[key];
            if (!(Array.isArray(value) && value.length === 0 || value === 0))
                fail("AUDIT_HAS_FAILURES");
        }
    const bindings = object(audit.fileHashes), bindingKeys = Object.keys(bindings);
    if (!bindingKeys.length || bindingKeys.length > 512)
        fail("AUDIT_BINDING_LIMIT");
    for (const path of bindingKeys) {
        safeRelative(path);
        if (hash(await read(path)) !== sha(bindings[path]))
            fail(`AUDIT_FILE_HASH_MISMATCH:${path}`);
    }
    const required = async (path) => {
        safeRelative(path);
        if (!(path in bindings))
            fail(`MISSING_AUDIT_BINDING:${path}`);
        return read(path);
    };
    ...
    same(protocol.strategy, S, "STRATEGY_MISMATCH");
    same(protocol.study, D, "STUDY_MISMATCH");
    if (protocol.historicalStrategyOutcomesComputedBeforeSeal !== false || protocol.independentHoldoutClaimed !== false
        || protocol.recentHistoryPreviouslyUsedByOtherCandidates !== true || protocol.parameterGrid !== false)
        fail("PROTOCOL_CLAIM_MISMATCH");
    if (report.strategyVersion !== S.version || report.researchPaperEligible !== true || report.provenProfitable !== false
        || report.independentValidationPassed !== false || report.liveTradingAllowed !== false)
        fail("NOT_RESEARCH_PAPER_ONLY");
    if ("provenProfitable" in audit && audit.provenProfitable !== false
        ...
        || "liveTradingAllowed" in audit && audit.liveTradingAllowed !== false)
        fail("AUDIT_PROFIT_CLAIM");
    ...
    if (!dataPath.startsWith("reports/"))
        fail("INVALID_DATA_SCOPE");
    ...
    if (typeof registeredData !== "string" || !(resolve(root, registeredData) === resolve(root, dataPath)
        || isAbsolute(registeredData) && registeredData.endsWith(`/${dataPath}`)))
        fail("DATA_DIRECTORY_MISMATCH");
    ...
    for (const file of ["source.json", "dataset.json", "manifest.json", "registration.json"])
        await required(`${dataPath}/${file}`);
    if (hash(await required(`${dataPath}/source.json`)) !== sha(report.sourceDataSha256)
        ...
        || hash(await required(`${dataPath}/manifest.json`)) !== sha(protocol.inputManifestSha256))
        fail("DATA_HASH_MISMATCH");
    ...
    if (data.bars[0].openMs > D.startMs - (S.movingAverageWeeks + 2) * 7 * 86_400_000
        || data.bars.at(-1).endMs < D.endMsExclusive)
        fail("INCOMPLETE_DATA_SCOPE");
    const windows = [{ id: "full", startMs: D.startMs, endMs: D.endMsExclusive }, ...D.periods];
    const runEntries = array(report.runs);
    if (runEntries.length !== windows.length * 2 * D.policies.length)
        fail("INCOMPLETE_RUN_SET");
    ...
    for (const raw of runEntries) {
        ...
        if (summaries.has(file))
            fail("DUPLICATE_OR_INVALID_RUN");
        ...
        for (const scenario of ["base", "stress"])
            for (const policy of D.policies) {
                ...
                if (finalCash < 0 || !near(finalCash - S.initialCashUsd, net) || !near(realized, net) || number(run.feesUsd) < 0
                    || number(run.maxWeeklyCloseDrawdownUsd) < 0 || typeof run.accountDrawdownHalted !== "boolean")
                    fail("RUN_ACCOUNTING_MISMATCH");
                const episodes = integer(run.closedEpisodes), buys = integer(run.buys), sells = integer(run.sells);
                if (episodes > Math.min(buys, sells) || orderEntries.length !== buys + sells)
                ...
                if (!near(number(run.fivePercentInitialBudgetHurdleUsd), hurdle)
                    || !near(number(run.netAboveAllocatedCapitalHurdleUsd), net - hurdle))
                    fail("RUN_HURDLE_MISMATCH");
                loaded.set(file, run);
            }
    ...
    if (completeWeeks !== base.weekly.length || integer(bootstrap.blocks) !== Math.max(0, completeWeeks - D.bootstrap.blockWeeks + 1)
        || report.lowerBootstrapMeanPositive !== (lower !== null && number(lower) > 0))
        fail("BOOTSTRAP_SUMMARY_MISMATCH");
    if ("bootstrapLowerMeanWeeklyNetUsd" in audit && audit.bootstrapLowerMeanWeeklyNetUsd !== lower)
        fail("AUDIT_BOOTSTRAP_MISMATCH");
    ...
    return { evidenceSha256: hash(JSON.stringify({ protocolSha256: hash(protocolBytes), reportSha256, auditSha256 })),
        reportSha256, auditSha256, sourceHashes: sourceHashes,
        summary: { baseNetUsd: number(base.netPnlUsd), stressNetUsd: number(stress.netPnlUsd), baseEpisodes: integer(base.closedEpisodes),
            stressEpisodes: integer(stress.closedEpisodes), lowerMeanWeeklyNetUsd: lower, provenProfitable: false } };
}
