...
import { bootstrapSpotWeeks, replaySpotTrend } from "./replay.js";
...
export async function runSpotStudy(dataRoot, output) {
    ...
    for (const source of sources) {
        const file = join(output, "sources", source.path);
    ...
    if (data.bars[0].openMs > D.startMs - (S.movingAverageWeeks + 2) * 7 * 86_400_000
        ...
        throw new Error("SPOT_STUDY_INCOMPLETE_HISTORY_SCOPE");
    ...
    for (const window of windows)
        for (const scenario of ["base", "stress"])
            for (const policy of D.policies) {
                ...
                await save(join(output, file), result);
                runs.push({ window: window.id, file, result });
                if (window.id === "full" && policy !== "cash")
                    process.stdout.write(JSON.stringify({ window: window.id, scenario, policy,
    ...
    const bootstrap = bootstrapSpotWeeks(baseTrend.weekly.map(w => w.weeklyNetUsd), D.bootstrap.blockWeeks, D.bootstrap.repetitions, D.bootstrap.seed, D.bootstrap.lowerQuantile);
    const checks = {
    ...
    for (const source of sources)
        if (sha(await readFile(source.path)) !== sourceHashes[source.path])
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
    if (process.argv.length !== 4)
        throw new Error("Usage: npx tsx src/spot-trend/study-main.ts DATA_DIRECTORY NEW_OUTPUT_DIRECTORY");
    await runSpotStudy(process.argv[2], process.argv[3]);
}
//# sourceMappingURL=study-main.js.map