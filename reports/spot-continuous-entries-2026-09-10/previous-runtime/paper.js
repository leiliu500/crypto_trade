import { createSpotAccount, markSpotAccount } from "./account.js";
import { WEEK_MS } from "./data.js";
import { reconstructSpotTrend } from "./signal.js";
import { SPOT_TREND_SPEC as S } from "./spec.js";
import { submitSpotPaperOrder, executeSpotPaperOrder, cancelSpotPaperOrder, validateSpotOrderLedger } from "./orders.js";
...
export function createSpotPaperState(evidenceSha256, nowMs) {
...
export function validateSpotPaperState(state) {
    ...
    if ((state.account.receipts.at(-1)?.timestampMs ?? 0) > state.lastCycleMs)
        throw new Error("SPOT_RECEIPT_AFTER_CYCLE");
}
export function prepareSpotPaperCycle(state, snapshot, nowMs) {
    validateSpotPaperState(state);
    if (!Number.isSafeInteger(nowMs) || nowMs < state.startedAtMs)
        throw new Error("INVALID_SPOT_PAPER_CLOCK");
    if (nowMs < state.lastCycleMs)
        throw new Error("SPOT_PAPER_REVERSED_CLOCK");
    if (nowMs === state.lastCycleMs) {
    ...
    if (pending) {
        const decision = { timestampMs: nowMs, action: pending.request.side,
    ...
    const unchanged = (reason) => {
        const decision = { timestampMs: nowMs, action: "hold", reason, fill: null, signal, mark: null };
    ...
    let action = "hold", reason = "HOLD_CASH", quantity;
    if (account.quantity > 0) {
        if (halted || !historyAvailable || signal.state === "cash") {
            action = "sell";
            reason = halted ? "ACCOUNT_DRAWDOWN_HALT" : !historyAvailable ? "HISTORY_UNAVAILABLE_EXIT" : "WEEKLY_TREND_EXIT";
        }
        else {
        ...
        if (account.receipts.some(fill => Math.floor(fill.timestampMs / WEEK_MS) * WEEK_MS === weekOpenMs))
            reason = "WEEKLY_ENTRY_ALREADY_CONSUMED";
        else if (nowMs - weekOpenMs >= SPOT_PAPER_SPEC.entryWindowMs)
            reason = "NEXT_WEEK_ENTRY_WINDOW";
        else {
            action = "buy";
            reason = "WEEKLY_TREND_ENTER";
        }
    }
    else
        reason = signal.reason;
    let orders = state.orders, orderId = null;
    if (action !== "hold") {
        const budgetUsd = Math.min(account.cashUsd, S.maximumEntryPrincipalUsd, S.entryPrincipalEquityFraction * Math.max(0, mark.liquidationEquityUsd));
        if (action === "buy" && budgetUsd <= 0) {
            action = "hold";
            reason = "NO_SPOT_ENTRY_CASH";
        }
        else {
            ...
            if (!Number.isFinite(requestedQuantity) || requestedQuantity < snapshot.rules.minimumQuantity
                ...
                reason = `${reason}:BELOW_MINIMUM_EXECUTABLE_ORDER`;
                action = "hold";
            }
            else {
                const request = { clientOrderId: `${SPOT_PAPER_SPEC.version}:${state.startedAtMs}:${state.cycles + 1}:${action}`,
    ...
    const decision = { timestampMs: nowMs, action, reason, fill: null, orderId, signal, mark };
...
export function settleSpotPaperCycle(state, snapshot, nowMs) {
    validateSpotPaperState(state);
    if (!Number.isSafeInteger(nowMs) || nowMs < state.lastCycleMs || !state.lastDecision)
        throw new Error("INVALID_SPOT_SETTLEMENT_CLOCK");
    const pending = state.orders.find(order => ["SUBMITTED", "ACCEPTED"].includes(order.status));
    if (!pending)
        return { state, decision: state.lastDecision };
    const weekOpenMs = Math.floor(nowMs / WEEK_MS) * WEEK_MS;
    ...
    const decision = { ...state.lastDecision, timestampMs: nowMs, signal, fill, orderId: result.order.orderId,
...
export function advanceSpotPaper(state, snapshot, nowMs) {
