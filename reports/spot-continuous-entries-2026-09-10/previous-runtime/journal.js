...
export async function durableSpotFile(file, bytes, exclusive) {
 ...
 */
export async function acquireSpotPaperLock(root) {
    ...
    try {
        await new Promise((resolveLocked, reject) => {
            ...
            let errorText = "";
            child.stderr?.on("data", (chunk) => { errorText += chunk.toString("utf8").slice(0, 1024); });
