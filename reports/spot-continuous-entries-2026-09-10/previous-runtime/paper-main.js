...
import { createSpotPaperState, prepareSpotPaperCycle, settleSpotPaperCycle, validateSpotPaperState, SPOT_PAPER_SPEC } from "./paper.js";
...
export async function loadSpotPaperEnvelope(file, root) {
    let bytes;
    try {
    ...
    if (envelope.version !== "spot-paper-journal-v1" || !/^[a-f0-9]{64}$/.test(envelope.runtimeSourceSha256)
        || !envelope.receiptEvidence || typeof envelope.receiptEvidence !== "object")
        throw new Error("INVALID_SPOT_JOURNAL");
    if (String(envelope.state.version) === LEGACY_SPOT_VERSION)
        migrateLegacySpotState(envelope.state, envelope.runtimeSourceSha256);
    else
        validateSpotPaperState(envelope.state);
    const verify = async (proof) => {
        if (!/^(?:cycles\/\d+-\d+(?:-(?:submitted|settled))?|migrations\/[a-f0-9]{64})\.json$/.test(proof.file)
            ...
            throw new Error("INVALID_SPOT_CYCLE_PATH");
        const raw = await readFile(join(root, proof.file));
        if (sha(raw) !== proof.sha256)
            throw new Error("SPOT_CYCLE_EVIDENCE_CHANGED");
        return JSON.parse(raw.toString("utf8"));
    };
    const verifySubmission = async (cycle) => {
        if (cycle.phase !== "BROKER_SETTLEMENT")
            return;
        if (!cycle.submittedEvidence)
            throw new Error("SPOT_MISSING_SUBMISSION_EVIDENCE");
        const submitted = await verify(cycle.submittedEvidence);
        if (!isDeepStrictEqual(submitted.after, cycle.before))
            throw new Error("SPOT_SUBMISSION_STATE_MISMATCH");
    ...
    if (envelope.lastEvidence) {
        ...
        if (JSON.stringify(cycle.after) !== JSON.stringify(envelope.state))
            throw new Error("SPOT_LAST_CYCLE_STATE_MISMATCH");
    }
    else if (envelope.state.cycles !== 0)
        throw new Error("SPOT_MISSING_CYCLE_EVIDENCE");
    if (Object.keys(envelope.receiptEvidence).length !== envelope.state.account.receipts.length)
        throw new Error("SPOT_RECEIPT_PROOF_COUNT");
    for (const fill of envelope.state.account.receipts) {
        const proof = envelope.receiptEvidence[fill.id];
        if (!proof)
            throw new Error("SPOT_MISSING_RECEIPT_EVIDENCE");
        const cycle = await verify(proof);
        if (cycle.phase !== "BROKER_SETTLEMENT")
            throw new Error("SPOT_FILL_WITHOUT_BROKER_SETTLEMENT");
        await verifySubmission(cycle);
        if (JSON.stringify(cycle.decision.fill) !== JSON.stringify(fill))
            throw new Error("SPOT_RECEIPT_EVIDENCE_MISMATCH");
    }
    if (envelope.migrationEvidence)
        await verify(envelope.migrationEvidence);
    return envelope;
}
export async function migrateSpotPaperEnvelope(envelope, root, sourceSha256) {
    if (!/^[a-f0-9]{64}$/.test(sourceSha256))
        throw new Error("INVALID_SPOT_MIGRATION_SOURCE_HASH");
    ...
    if (!isDeepStrictEqual(JSON.parse(oldBytes.toString("utf8")), envelope))
        throw new Error("SPOT_STATE_CHANGED_DURING_MIGRATION");
    const oldSha = sha(oldBytes), proofFile = `migrations/${oldSha}.json`;
    await mkdir(join(root, "migrations"), { recursive: true });
    const backup = join(root, `state-v1-${oldSha}.json`);
    try {
        await durableFile(backup, oldBytes.toString("utf8"), true);
    }
    catch (error) {
        if (error.code !== "EEXIST" || sha(await readFile(backup)) !== oldSha)
            throw error;
    }
    let proofBytes;
    try {
        proofBytes = await readFile(join(root, proofFile), "utf8");
    }
    catch (error) {
        if (error.code !== "ENOENT")
            throw error;
    ...
    if (proof.version !== "spot-paper-order-migration-v1" || !Number.isSafeInteger(proof.migratedAtMs)
        || proof.migratedAtMs < 0 || proof.beforeEnvelopeSha256 !== oldSha || proof.beforeRuntimeSha256 !== envelope.runtimeSourceSha256
        ...
        || proof.accountChanged !== false)
        throw new Error("SPOT_MIGRATION_EVIDENCE_MISMATCH");
    const migrationEvidence = { file: proofFile, sha256: sha(proofBytes) };
    const migrated = { ...envelope, state, runtimeSourceSha256: sourceSha256,
...
export async function runSpotPaper(args = process.argv.slice(2)) {
    ...
    const [studyDirectory, auditFile, stateDirectory, mode] = args;
    ...
    await mkdir(join(root, "cycles"), { recursive: true });
    ...
    try {
        ...
        let lastError = envelope.state.lastDecision?.mark === null
        ...
        if (mode === "--serve") {
            server = createServer((request, response) => {
                if (!["/", "/status", "/healthz"].includes(request.url ?? "")) {
                    response.writeHead(404);
                    response.end();
                    return;
                }
                const result = status();
                response.writeHead(request.url === "/healthz" && !result.healthy ? 503 : 200, { "Content-Type": "application/json", "Cache-Control": "no-store" });
                response.end(JSON.stringify(result));
            });
            await new Promise((resolveReady, reject) => { server.once("error", reject); server.listen(3002, "0.0.0.0", resolveReady); });
        }
        do {
            ...
            try {
                ...
                if (advanced.state === before)
                    throw new Error("SPOT_DUPLICATE_CYCLE_CLOCK");
                ...
                const next = { ...envelope, state: advanced.state, lastEvidence: proof,
                ...
                if (!stopping && envelope.state.orders.some(order => ["SUBMITTED", "ACCEPTED"].includes(order.status))) {
                    ...
                    const settledEnvelope = { ...envelope, state: settled.state, lastEvidence: settlementProof,
                ...
                if (!lastError)
                    lastSuccessMs = envelope.state.lastCycleMs;
                ...
                if (persistenceStarted || mode === "--once")
                    throw error;
            }
            if (mode === "--once" || stopping)
                break;
            await new Promise(resolveWait => {
                const timer = setTimeout(resolveWait, SPOT_PAPER_SPEC.cycleIntervalMs);
                wake = () => { clearTimeout(timer); resolveWait(); };
                if (stopping)
                    wake();
            });
        } while (!stopping);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href)
    await runSpotPaper();
//# sourceMappingURL=paper-main.js.map