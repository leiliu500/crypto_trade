...
import { applySpotFill, markSpotAccount, planSpotPaperFill } from "./account.js";
...
const timestamp = (value) => finite(value) && Number.isSafeInteger(value) && value >= 0
...
const aligned = (value, step) => positive(value) && positive(step)
    && Number.isSafeInteger(Math.round(value / step)) && near(value / step, Math.round(value / step));
const orderIdFor = (request) => `paper-spot-${createHash("sha256").update(request.clientOrderId).digest("hex").slice(0, 32)}`;
const fillIdFor = (order) => order.request.clientOrderId;
const sameFill = (a, b) => a.id === b.id && a.side === b.side && a.quantity === b.quantity
    && a.price === b.price && a.feeBps === b.feeBps && a.timestampMs === b.timestampMs;
const sameRequest = (a, b) => REQUEST_KEYS.every(key => a[key] === b[key]);
...
function invalidRequest(request) {
    ...
    if (!positive(request.quantity) || !positive(request.limitPrice))
        return "INVALID_QUANTITY_OR_PRICE";
    if (request.feeBps < 0 || request.feeBps >= 10_000)
        return "INVALID_FEE";
    if (request.reduceOnly !== (request.side === "sell"))
        return "INVALID_REDUCE_ONLY";
    const notional = request.quantity * request.limitPrice;
    if (!positive(notional) || !positive(notional + notional * request.feeBps / 10_000))
        return "INVALID_ORDER_ARITHMETIC";
    return null;
}
function accountRejection(request, account) {
    if (request.createdAtMs < (account.receipts.at(-1)?.timestampMs ?? 0))
        return "REVERSED_REQUEST_TIME";
    if (request.side === "buy") {
        const notional = request.quantity * request.limitPrice;
        if (notional + notional * (request.feeBps / 10_000) > account.cashUsd)
            return "INSUFFICIENT_CASH_AT_LIMIT";
    }
    else if (request.quantity > account.quantity)
...
export function validateSpotOrderLedger(orders, account) {
    validateAccount(account);
    if (!Array.isArray(orders))
        throw new Error("SPOT_INVALID_ORDER_LEDGER");
    const clients = new Set(), ids = new Set(), fills = new Map();
    let pending = 0;
    for (const order of orders) {
        validateOrder(order);
        if (clients.has(order.request.clientOrderId) || ids.has(order.orderId))
            throw new Error("SPOT_DUPLICATE_ORDER_ID");
        clients.add(order.request.clientOrderId);
        ids.add(order.orderId);
        if (["SUBMITTED", "ACCEPTED"].includes(order.status) && ++pending > 1)
            throw new Error("SPOT_MULTIPLE_PENDING_ORDERS");
        if (order.fill)
            fills.set(order.fill.id, order.fill);
    }
    if (fills.size !== account.receipts.length)
        throw new Error("SPOT_ORDER_RECEIPT_COUNT_MISMATCH");
    for (const receipt of account.receipts) {
        const fill = fills.get(receipt.id);
        if (!fill || !sameFill(fill, receipt))
...
export function submitSpotPaperOrder(existingOrders, request, account) {
    ...
    if (previous) {
        if (!sameRequest(previous.request, request))
            throw new Error("SPOT_CONFLICTING_CLIENT_ORDER_ID");
    ...
    const order = { request: { ...request }, orderId: orderIdFor(request), status: reason ? "REJECTED" : "SUBMITTED",
        ...
        events: [{ type: "SUBMITTED", timestampMs: request.createdAtMs },
            ...(reason ? [{ type: "REJECTED", timestampMs: request.createdAtMs, detail: reason }] : [])], fill: null };
    validateOrder(order);
    return [...existingOrders, order];
}
/** Explicit cancellation is durable through the same terminal event shape as IOC expiry. */
export function cancelSpotPaperOrder(order, reason, nowMs) {
    validateOrder(order);
    ...
    const events = [...order.events,
        ...(order.status === "SUBMITTED" ? [{ type: "ACCEPTED", timestampMs: nowMs }] : []),
        { type: "CANCELED", timestampMs: nowMs, detail: reason }];
    const canceled = { ...order, status: "CANCELED", cancellationReason: reason, events };
    validateOrder(canceled);
    return canceled;
}
/** Limit filtering removes depth; this cap never invents additional displayed liquidity. */
function boundedDepth(levels, request, lotSize) {
    ...
    for (const [price, displayed] of levels) {
        if (request.side === "buy" ? price > request.limitPrice : price < request.limitPrice)
            break;
...
export function executeSpotPaperOrder(order, account, snapshot, nowMs) {
    validateAccount(account);
    validateOrder(order);
    ...
    if (previousFill)
        throw new Error("SPOT_PENDING_ORDER_ALREADY_HAS_RECEIPT");
    if (!timestamp(nowMs) || nowMs < order.events.at(-1).timestampMs
        || nowMs < (account.receipts.at(-1)?.timestampMs ?? 0))
        throw new Error("SPOT_INVALID_EXECUTION_CLOCK");
    const accepted = order.status === "ACCEPTED" ? order : { ...order, status: "ACCEPTED",
        events: [...order.events, { type: "ACCEPTED", timestampMs: nowMs }] };
    const cancel = (reason) => {
        const canceled = { ...accepted, status: "CANCELED", cancellationReason: reason,
            events: [...accepted.events, { type: "CANCELED", timestampMs: nowMs, detail: reason }] };
        validateOrder(canceled);
        return { order: canceled, account };
    };
    if (nowMs - order.request.createdAtMs > SPOT_ORDER_SPEC.maximumLifetimeMs)
        return cancel("REQUEST_EXPIRED");
    if (!snapshot || !finite(snapshot.feeBps) || snapshot.feeBps !== order.request.feeBps)
        return cancel("FEE_POLICY_MISMATCH");
    ...
    if (accountReason)
        return cancel(accountReason);
    if (!rules || ![rules.lotSize, rules.minimumQuantity, rules.tickSize].every(positive)
        || !finite(rules.minimumNotionalUsd) || rules.minimumNotionalUsd < 0)
        return cancel("INVALID_RULES");
    if (!aligned(request.quantity, rules.lotSize) || !aligned(request.limitPrice, rules.tickSize))
        return cancel("ORDER_RULE_STEP_MISMATCH");
    ...
    if (!probe.fill && probe.reason !== "BELOW_MINIMUM_EXECUTABLE_ORDER")
        return cancel(probe.reason);
    const levels = boundedDepth(request.side === "buy" ? book.asks : book.bids, request, rules.lotSize);
    if (!levels.length)
        return cancel("IOC_LIMIT_NOT_MARKETABLE");
    const limitedBook = { ...book, ...(request.side === "buy" ? { asks: levels } : { bids: levels }) };
    ...
    const filled = { ...accepted, status: partial ? "CANCELED" : "FILLED", fill,
        ...
        events: [...accepted.events, { type: partial ? "PARTIAL_FILL" : "FILLED", timestampMs: nowMs },
            ...(partial ? [{ type: "CANCELED", timestampMs: nowMs, detail: "IOC_UNFILLED_REMAINDER" }] : [])] };
