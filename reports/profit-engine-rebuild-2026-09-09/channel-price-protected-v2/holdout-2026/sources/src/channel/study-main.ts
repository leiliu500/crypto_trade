...
import { replayChannel, replayPriceProtectedChannel } from "./replay.js";
...
const sourcePaths = ["src/channel/spec.ts", "src/channel/replay.ts", "src/channel/study-main.ts", "test/channel-replay.test.ts", "test/channel-study.test.ts",
  "src/channel/verify-v1-parity-main.ts", "test/channel-price-protection.test.ts",
...
export async function runChannelStudy(output: string, variant: "classic-v1" | "price-protected-v2" = "classic-v1") {
  if (!["classic-v1", "price-protected-v2"].includes(variant)) throw new Error("INVALID_CHANNEL_STUDY_VARIANT");
  const protectedVariant = variant === "price-protected-v2", strategy = protectedVariant ? P : S;
  let v1ParitySha256: string | null = null;
  if (protectedVariant) {
    const bytes = await readFile(join(output, "v1-parity.json"));
    const parity = JSON.parse(bytes.toString("utf8")) as { allOriginalRunsIdentical: boolean; sourceHashes: Record<string, string> };
    if (parity.allOriginalRunsIdentical !== true) throw new Error("CHANNEL_V1_PARITY_REQUIRED_BEFORE_V2");
    for (const [path, hash] of Object.entries(parity.sourceHashes))
      if (SHA(await readFile(path)) !== hash) throw new Error(`CHANNEL_SOURCE_CHANGED_AFTER_V1_PARITY:${path}`);
    v1ParitySha256 = SHA(bytes);
  }
  ...
  await save(join(output, "protocol.json"), { frozenAtUtc: new Date().toISOString(), strategy, study: D, variant,
    v1ParitySha256,
  ...
  for (const window of D.windows) for (const scenario of D.scenarios)
    for (const policy of ["channel", "buy-hold-btc", "buy-hold-eth"] as const) {
      const run = (protectedVariant ? replayPriceProtectedChannel : replayChannel)({ bars, funding, startMs: window.startMs, endMs: window.endMs, scenario, policy });
  ...
  const report = { generatedAtUtc: new Date().toISOString(), strategyVersion: strategy.version, variant, dataSha256,
  ...
  await writeFile(join(output, "report.md"), [
    `The fixed daily 55/20 channel candidate ${strategy.version} ${report.historicalDevelopmentEligible ? "passed" : "failed"} its declared historical development screen. Runtime activation remains false.`, "",
    ...
    ...(protectedVariant ? ["V2 introduces a fixed stop anchored to the completed signal close and rejects fills displaced by more than half of the signal ATR. This is a new hypothesis informed by v1's observed late-entry failure, not a correction to v1 or independent validation. All original v1 run files were reproduced byte-for-byte before the v2 replay.", ""] : []),
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.argv.length < 3 || process.argv.length > 4 || process.argv[3] !== undefined && process.argv[3] !== "price-protected-v2")
    throw new Error("Usage: npx tsx src/channel/study-main.ts NEW_OUTPUT_DIRECTORY [price-protected-v2]");
  await runChannelStudy(process.argv[2]!, process.argv[3] === "price-protected-v2" ? "price-protected-v2" : "classic-v1");
}
