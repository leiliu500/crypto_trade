...
import { CHANNEL_SPEC as S } from "./spec.js";
...
export function replayChannel(input: ChannelReplayInput) {
  ...
  if (!Number.isSafeInteger(startMs) || !Number.isSafeInteger(endMs) || startMs % D || endMs % D || endMs <= startMs
    || endMs > Date.UTC(2025, 6, 1) || !["base", "stress"].includes(scenario)
  ...
  for (let t = startMs; t < endMs; t += H) {
    ...
    for (const symbol of S.symbols) {
      ...
      const side = s.entrySide!, px = price(b.open, side, symbol), stopDistance = S.stopAtr * s.atr;
      ...
      positions.set(symbol, { symbol, side, qty, entryQty: qty, entryPx: px, entryMs: t, stopPx: px - side * stopDistance,
      ...
      orders.push({ atMs: t, symbol, side, qty, price: px, feeUsd: fee, grossPnlUsd: 0, reduceOnly: false, reason: policy === "channel" ? "55_DAY_BREAKOUT" : "DESCRIPTIVE_BUY_HOLD" });
    }
  ...
  return { version: S.version, startMs, endMs, scenario, policy, accountingKnown: known, netPnlUsd,
