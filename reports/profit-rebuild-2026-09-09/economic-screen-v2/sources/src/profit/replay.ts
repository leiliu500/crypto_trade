...
import type { ProfitBar, ProfitForecast, ProfitModelFit, ProfitSymbol } from "./model.js";
import { PROFIT_SPEC as S } from "./spec.js";
import { evaluateProfitUtility, PROFIT_UTILITY_SPEC } from "./utility.js";
...
export type ProfitReplayPolicy = "weekly-forecast" | "weekly-mean-variance" | "risk-managed-long-btc" | "risk-managed-long-eth";
interface Candidate { forecast: ProfitForecast; side: Direction; netLowerBps: number; score: number; maximumEquityFraction?: number }
...
export interface ProfitReplayInput {
  ...
  /** Required only by the separately versioned mean-variance research policy. */
...
export function replayProfit(input: ProfitReplayInput) {
  ...
  if (![input.startMs, input.endMs].every(v => Number.isSafeInteger(v) && v >= 0 && v % DAY === 0)
    ...
    || !["weekly-forecast", "weekly-mean-variance", "risk-managed-long-btc", "risk-managed-long-eth"].includes(policy)) throw new Error("INVALID_PROFIT_REPLAY_INPUT");
  const utilityPolicy = policy === "weekly-mean-variance", fits = new Map<string, ProfitModelFit>();
  if (utilityPolicy) {
    if (!Array.isArray(input.fits)) throw new Error("PROFIT_UTILITY_MODEL_FITS_REQUIRED");
    for (const fit of input.fits) {
      if (!fit || typeof fit.id !== "string" || fits.has(fit.id)) throw new Error("PROFIT_UTILITY_INVALID_OR_DUPLICATE_FIT");
      fits.set(fit.id, fit);
    }
  }
  ...
  const candidatesAt = (decisionMs: number): Candidate[] => {
    ...
    for (const f of group) {
      ...
      if (policy === "risk-managed-long-btc" || policy === "risk-managed-long-eth") {
      ...
      if (utilityPolicy) {
        const fit = fits.get(f.modelId), utility = fit ? evaluateProfitUtility(fit, f, hurdleBps) : null;
        if (!utility) { block(fit ? "NO_POSITIVE_VALID_NET_MEAN_UTILITY" : "UTILITY_MODEL_FIT_UNAVAILABLE"); continue; }
        candidates.push({ forecast: f, side: utility.side, netLowerBps: utility.netMeanBps,
          score: utility.score, maximumEquityFraction: utility.maximumEquityFraction });
        continue;
      }
  ...
  for (let atMs = input.startMs; atMs < input.endMs; atMs += HOUR) {
    ...
    if (p && !exitReason) {
      ...
      if (target && target.forecast.symbol === p.symbol && target.side === p.side) {
        ...
        const desiredForecastQty = Math.min(desiredRiskQty, target.maximumEquityFraction === undefined ? Infinity
          : floorQty(Math.max(0, equity) * target.maximumEquityFraction / bar.open, p.symbol));
        if (desiredForecastQty < p.qty * (1 - S.quantityDeadbandFraction)) desired = Math.min(desired, desiredForecastQty);
      }
    ...
    if (!p && target && atMs >= entryAfterMs) {
      ...
      if (age < 0 || age > S.maximumSignalAgeMs) { block("FORECAST_EXPIRED"); target = undefined; }
        ...
        const equity = A.initialEquityUsd + cash, cap = Math.min(S.maximumNotionalUsd, equity * S.maximumEquityFraction,
          equity * (target.maximumEquityFraction ?? S.maximumEquityFraction));
        ...
        if (!(qty > 0)) { block("QUANTITY_BELOW_MINIMUM_LOT"); target = undefined; }
          ...
          orders.push({ symbol: f.symbol, side: target.side, atMs, qty, price: px, reduceOnly: false,
            reason: policy === "weekly-forecast" ? "POSITIVE_CONSERVATIVE_NET_FORECAST"
              : utilityPolicy ? "POSITIVE_NET_MEAN_VARIANCE_FORECAST" : "BENCHMARK_LONG_ENTRY",
  ...
  return {
    version: utilityPolicy ? PROFIT_UTILITY_SPEC.version : S.version, policy,
    benchmarkOnly: policy === "risk-managed-long-btc" || policy === "risk-managed-long-eth",
