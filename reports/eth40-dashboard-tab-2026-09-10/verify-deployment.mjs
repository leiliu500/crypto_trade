...
for(let i=0;i<containers.length;i++){
 const c=containers[i];assert.equal(hash([...c.Config.Env].sort()),before.containers[i].environmentSha256);
 assert.deepEqual(c.Mounts.map(m=>({name:m.Name,destination:m.Destination,readWrite:m.RW})),before.containers[i].mounts);
...
assert.equal(futures.overall,'healthy');for(const key of ['mode','paper','strategyVersion','configurationVersion','equity','sessionStartingEquity'])assert.deepEqual(futures[key],oldFutures[key],key);
