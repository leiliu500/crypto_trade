...
const trusted = new WeakSet<SpotAccount>();
...
function seal(account: SpotAccount): SpotAccount {
  for (const receipt of account.receipts) Object.freeze(receipt);
...
export function createSpotAccount(initialCashUsd = 100_000): SpotAccount {
  if (!positive(initialCashUsd)) throw new Error("SPOT_INVALID_INITIAL_CASH");
  return seal({ initialCashUsd, cashUsd: initialCashUsd, quantity: 0, entryCostUsd: 0,
    realizedNetUsd: 0, feesUsd: 0, receipts: [] });
}

function checkFill(fill: SpotFill): void {
  if (!fill || !identifier(fill.id) || !["buy", "sell"].includes(fill.side)
    ...
    || !timestamp(fill.timestampMs)) throw new Error("SPOT_INVALID_FILL");
}

function sameFill(a: SpotFill, b: SpotFill): boolean {
  return a.id === b.id && a.side === b.side && a.quantity === b.quantity && a.price === b.price
    && a.feeBps === b.feeBps && a.timestampMs === b.timestampMs;
}

function transact(account: SpotAccount, fill: SpotFill): SpotAccount {
  ...
  if (!positive(notional) || !nonnegative(fee)) throw new Error("SPOT_ARITHMETIC_OVERFLOW");
  let cashUsd: number, quantity: number, entryCostUsd: number;
  ...
  if (fill.side === "buy") {
    ...
    if (debit > account.cashUsd) throw new Error("SPOT_INSUFFICIENT_CASH");
    ...
    if (fill.quantity > account.quantity) throw new Error("SPOT_INSUFFICIENT_INVENTORY");
...
function checkedAccount(account: SpotAccount): SpotAccount {
  if (!account || typeof account !== "object") throw new Error("SPOT_INVALID_ACCOUNT");
  if (trusted.has(account)) return account;
  if (!positive(account.initialCashUsd) || !Array.isArray(account.receipts)
    ...
    || !finite(account.realizedNetUsd)) throw new Error("SPOT_INVALID_ACCOUNT");
  let restored = createSpotAccount(account.initialCashUsd);
  const seen = new Set<string>();
  for (const fill of account.receipts) {
    checkFill(fill);
    if (seen.has(fill.id)) throw new Error("SPOT_DUPLICATE_RECEIPT");
    seen.add(fill.id);
    restored = transact(restored, fill);
  }
  const fields = ["cashUsd", "quantity", "entryCostUsd", "realizedNetUsd", "feesUsd"] as const;
  if (fields.some(field => !near(restored[field], account[field]))) throw new Error("SPOT_ACCOUNT_RECONCILIATION_FAILED");
  return restored;
}

export function applySpotFill(account: SpotAccount, fill: SpotFill): SpotAccount {
  const current = checkedAccount(account);
  checkFill(fill);
  const previous = current.receipts.find(receipt => receipt.id === fill.id);
  if (previous) {
    if (sameFill(previous, fill)) return current;
    throw new Error("SPOT_CONFLICTING_FILL_ID");
  }
  return transact(current, fill);
}

export function markSpotAccount(account: SpotAccount, bid: number, exitFeeBps: number): {
  equityUsd: number; liquidationEquityUsd: number; unrealizedNetUsd: number; netPnlUsd: number;
} {
  const current = checkedAccount(account);
  if (!positive(bid) || !nonnegative(exitFeeBps) || exitFeeBps >= 10_000) throw new Error("SPOT_INVALID_MARK");
...
function validDepth(levels: [number, number][], direction: 1 | -1, tickSize: number): boolean {
...
function sweep(levels: [number, number][], quantity: number): number | null {
 ...
 */
export function planSpotPaperFill(input: SpotPaperFillInput): SpotPaperFillPlan {
  let visibleNotionalUsd = 0;
  const deny = (reason: string): SpotPaperFillPlan => ({ fill: null, reason, visibleNotionalUsd });
  if (!input || !["buy", "sell"].includes(input.side) || !identifier(input.id)
    ...
    || input.quantity !== undefined && (!positive(input.quantity) || input.side !== "sell")) return deny("INVALID_INPUT");
  let account: SpotAccount;
  try { account = checkedAccount(input.account); } catch { return deny("INVALID_ACCOUNT"); }
  if (account.receipts.some(fill => fill.id === input.id)) return deny("FILL_ID_ALREADY_USED");
  if (input.nowMs < (account.receipts.at(-1)?.timestampMs ?? 0)) return deny("REVERSED_FILL_TIME");
  const { book, rules, side } = input;
  if (!rules || ![rules.lotSize, rules.minimumQuantity, rules.tickSize].every(positive)
    || !nonnegative(rules.minimumNotionalUsd)) return deny("INVALID_RULES");
  if (!book || !timestamp(book.receivedAtMs) || book.receivedAtMs > input.nowMs
    || input.nowMs - book.receivedAtMs > 5_000) return deny("STALE_OR_FUTURE_BOOK");
  if (!validDepth(book.bids, -1, rules.tickSize) || !validDepth(book.asks, 1, rules.tickSize)
    || book.bids[0]![0] >= book.asks[0]![0]) return deny("INVALID_BOOK");
  ...
  const quantityCap = Math.min(visibleQuantity * 0.05,
    side === "sell" ? Math.min(account.quantity, input.quantity ?? account.quantity) : Infinity);
  const lots = Math.floor(quantityCap / rules.lotSize);
  if (!Number.isSafeInteger(lots)) return deny("UNREPRESENTABLE_LOT_COUNT");
  ...
  const candidate = (count: number): SpotFill | null => {
    const quantity = count * rules.lotSize;
    if (!(quantity > 0) || quantity > quantityCap) return null;
    const price = sweep(levels, quantity);
    if (price === null || !positive(price)) return null;
    const notional = quantity * price, fee = notional * (input.feeBps / 10_000);
    if (!positive(notional) || !nonnegative(fee)) return null;
  ...
  try { applySpotFill(account, fill); } catch { return deny("UNREPRESENTABLE_FILL"); }
