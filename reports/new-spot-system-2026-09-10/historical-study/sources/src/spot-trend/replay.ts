import { applySpotFill, createSpotAccount, markSpotAccount, type SpotFill } from "./account.js";
import { WEEK_MS, type SpotWeek } from "./data.js";
import { reconstructSpotTrend, type SpotTrendSignal } from "./signal.js";
import { SPOT_TREND_SPEC as S, type SpotPolicy, type SpotScenario } from "./spec.js";

export interface SpotReplayWeek {
  openMs: number; endMs: number; signalAvailableAtMs: number; signalState: string; signalReason: string;
  quantity: number; cashUsd: number; liquidationEquityUsd: number; weeklyNetUsd: number;
  markedNotionalUsd: number; weeklyLowLiquidationEquityUsd: number;
}
export interface SpotReplay {
  version: string; policy: SpotPolicy; scenario: SpotScenario; startMs: number; endMs: number;
  firstEligibleExecutionOpenMs: number | null; finalEligibleExecutionOpenMs: number | null;
  initialCashUsd: number; initialEntryBudgetUsd: number; finalCashUsd: number; finalQuantity: number;
  netPnlUsd: number; realizedNetUsd: number; feesUsd: number; closedEpisodes: number;
  netReturnOnInitialEntryBudgetFraction: number; drawdownOnInitialEntryBudgetFraction: number; netAccountReturnFraction: number;
  buys: number; sells: number; turnoverUsd: number; investedWeeks: number;
  maximumMarkedNotionalUsd: number; maxWeeklyCloseDrawdownUsd: number; sampledPeakToWeeklyLowUsd: number;
  accountDrawdownHalted: boolean; terminalFlat: boolean;
  fivePercentInitialBudgetHurdleUsd: number; netAboveAllocatedCapitalHurdleUsd: number;
  orders: Array<SpotFill & { reason: string }>; weekly: SpotReplayWeek[];
}

export function replaySpotTrend(input: {
  bars: readonly SpotWeek[]; startMs: number; endMs: number; policy: SpotPolicy; scenario: SpotScenario;
}): SpotReplay {
  ...
  if (bars.length < 2) throw new Error("SPOT_REPLAY_TOO_SHORT");
  const costs = S.scenarios[input.scenario], orders: SpotReplay["orders"] = [], weekly: SpotReplayWeek[] = [];
  let account = createSpotAccount(S.initialCashUsd), peak: number = S.initialCashUsd, closePeak: number = S.initialCashUsd, priorEquity: number = S.initialCashUsd;
  ...
  const roundPrice = (price: number, side: "buy" | "sell") => {
  ...
  const floorQuantity = (q: number) => Math.floor(q / S.historicalLotSize) * S.historicalLotSize;
  const transact = (side: "buy" | "sell", quantity: number, bar: SpotWeek, reason: string) => {
    ...
    if (bar.volume === 0 || quantity < S.historicalMinimumQuantity || quantity * price < S.historicalMinimumNotionalUsd) return false;
    const fill: SpotFill = { id: `${S.version}:${input.policy}:${input.scenario}:${bar.openMs}:${orders.length}`,
    ...
    if (oldQuantity > 0 && account.quantity === 0) closedEpisodes++;
    orders.push({ ...fill, reason }); return true;
  };
  const flatten = (bar: SpotWeek, reason: string) => account.quantity > 0 && transact("sell", account.quantity, bar, reason);
  for (let i = 0; i < bars.length; i++) {
    const bar = bars[i]!, terminal = i === bars.length - 1;
    const signal: SpotTrendSignal = reconstructSpotTrend(input.bars, bar.openMs - costs.additionalDelayWeeks * WEEK_MS);
    ...
    if (input.policy === "trend" && openingMark.liquidationEquityUsd <= peak * (1 - S.maximumAccountDrawdownFraction)) halted = true;
    if (terminal) flatten(bar, "TERMINAL_LIQUIDATION");
    else if (input.policy === "trend" && (halted || signal.state === "cash")) flatten(bar, halted ? "ACCOUNT_DRAWDOWN_HALT" : signal.reason);
      ...
      if (account.quantity * bar.open > cap) {
        ...
        else flatten(bar, "MARKED_NOTIONAL_CAP_REMAINDER");
      }
      ...
      const budget = Math.min(account.cashUsd, S.maximumEntryPrincipalUsd,
        input.policy === "buy-hold" ? initialEntryBudgetUsd : S.entryPrincipalEquityFraction * Math.max(0, equity));
      const price = roundPrice(bar.open, "buy");
      const quantity = floorQuantity(budget / (price * (1 + costs.feeBps / 10_000)));
      if (transact("buy", quantity, bar, input.policy === "buy-hold" ? "BUY_HOLD_START" : signal.reason)) benchmarkEntered = true;
    }
    ...
    if (account.quantity > 0) investedWeeks++;
...
export function bootstrapSpotWeeks(values: readonly number[], blockWeeks: number, repetitions: number, seed: number, quantile: number) {
  if (!Number.isInteger(blockWeeks) || blockWeeks < 1 || !Number.isInteger(repetitions) || repetitions < 1
    || !Number.isFinite(quantile) || quantile <= 0 || quantile >= 1 || !values.every(Number.isFinite)) throw new Error("INVALID_SPOT_BOOTSTRAP");
  if (values.length < blockWeeks) return { lowerMeanWeeklyNetUsd: null, completeWeeks: values.length, blocks: 0 };
  let random = seed >>> 0;
  const next = () => { random = (Math.imul(random, 1664525) + 1013904223) >>> 0; return random / 2 ** 32; };
  const means: number[] = [], blocks = values.length - blockWeeks + 1;
  for (let r = 0; r < repetitions; r++) {
    let sum = 0, count = 0;
    while (count < values.length) {
      const start = Math.floor(next() * blocks);
      for (let j = 0; j < blockWeeks && count < values.length; j++, count++) sum += values[start + j]!;
    }
