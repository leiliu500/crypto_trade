export type HaltReason = "SEQUENCE_GAP" | "BOOK_INVALID" | "PUBLIC_STREAM_DOWN" | "PRIVATE_STREAM_DOWN" | "ACCOUNT_UNKNOWN" | "ORDER_SEND_UNKNOWN" | "ACK_LATENCY" | "ROLLING_LOSS" | "DRAWDOWN" | "CLOCK_INVALID" | "PROCESS_STALL" | "PERSISTENCE_UNAVAILABLE" | "AUDIT_DATA_LOSS" | "MANUAL";
export interface HealthState { publicStream: boolean; privateStream: boolean; accountReconciled: boolean; bookValid: boolean; clockValid: boolean; riskRecomputed: boolean; persistenceReady: boolean; }

export class RiskState {
  private readonly haltReasons = new Set<HaltReason>();
  private health: HealthState = { publicStream: false, privateStream: false, accountReconciled: false, bookValid: false, clockValid: true, riskRecomputed: false, persistenceReady: true };
  private equity = 0;
  ...
  public resumeAfterReconciliation(): boolean {
    ...
    // Lost audit records require an explicit recovery outside automatic health
    // reconciliation. A reconnected database alone cannot remove that latch.
    const operational: HaltReason[] = ["SEQUENCE_GAP", "BOOK_INVALID", "PUBLIC_STREAM_DOWN", "PRIVATE_STREAM_DOWN", "ACCOUNT_UNKNOWN", "ORDER_SEND_UNKNOWN", "ACK_LATENCY", "CLOCK_INVALID", "PROCESS_STALL", "PERSISTENCE_UNAVAILABLE"];
