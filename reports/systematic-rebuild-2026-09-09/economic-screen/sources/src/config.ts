...
import { createDistributionSizingPolicy, distributionSizingStateFile, type DistributionSizingPolicy } from "./distribution/sizing.js";
...
export interface EngineConfig extends Omit<SymbolConfig, "symbol"> {
  ...
  distributionalSizingPolicy?: DistributionSizingPolicy;
  ...
  databaseMaximumWriteLagMs: number;
...
const GLOBAL_PARAMETER_KEYS = new Set([
  "DISTRIBUTIONAL_MAXIMUM_NOTIONAL", "DISTRIBUTIONAL_MAXIMUM_EQUITY_FRACTION",
  ...
  "DATABASE_MAXIMUM_WRITE_LAG_MS", "DATABASE_STATEMENT_TIMEOUT_MS",
  "MAXIMUM_GROSS_NOTIONAL", "RECALL_SAMPLE_INTERVAL_MS", "RECALL_OPPORTUNITY_HORIZON_MS", "RECALL_MIN_NET_MOVE_BPS",
  "MAXIMUM_CLUSTER_POSITIONS", "ROLLING_LOSS_FRACTION", "SESSION_LOSS_FRACTION", "MAXIMUM_DRAWDOWN_FRACTION",
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env, modeOverride?: string): EngineConfig {
  ...
  const makerFeeBps = paperEntryExercise ? 0 : validateFeeBps(numberEnv(env.KRAKEN_FUTURES_MAKER_FEE_BPS, 2), "KRAKEN_FUTURES_MAKER_FEE_BPS");
  const takerFeeBps = paperEntryExercise ? 0 : validateFeeBps(numberEnv(env.KRAKEN_FUTURES_TAKER_FEE_BPS, 5), "KRAKEN_FUTURES_TAKER_FEE_BPS");
  ...
  const distributionalBaseStateFile = configuredEnv.DISTRIBUTIONAL_STATE_FILE ?? "data/distributional-state.json";
  const sizingMode = configuredEnv.DISTRIBUTIONAL_SIZING_MODE ?? "RISK_BOUNDED";
  if (sizingMode !== "RISK_BOUNDED" && sizingMode !== "LEGACY_FIXED") throw new Error("INVALID_DISTRIBUTIONAL_SIZING_MODE");
  const distributionalSizingPolicy = sizingMode === "RISK_BOUNDED" ? createDistributionSizingPolicy(symbolConfigs,
    positiveNumberEnv(configuredEnv.DISTRIBUTIONAL_MAXIMUM_NOTIONAL, 1_000),
    positiveFractionEnv(configuredEnv.DISTRIBUTIONAL_MAXIMUM_EQUITY_FRACTION, .01)) : undefined;
  const distributionalStateFile = distributionSizingStateFile(distributionalBaseStateFile, distributionalSizingPolicy);
  const distributionalHistoryFile = configuredEnv.DISTRIBUTIONAL_HISTORY_FILE ?? `${distributionalBaseStateFile}.market.json`;
  const distributionalTrainingFile = configuredEnv.DISTRIBUTIONAL_TRAINING_FILE?.trim() || undefined;
  if (distributionalTrainingFile && [distributionalBaseStateFile, `${distributionalBaseStateFile}.pending`, `${distributionalBaseStateFile}.tmp`,
    `${distributionalBaseStateFile}.pending.tmp`, distributionalStateFile, `${distributionalStateFile}.pending`, `${distributionalStateFile}.tmp`,
  ...
  if (parseBoolean(configuredEnv.DISTRIBUTIONAL_ENGINE_ENABLED, false)
    && (!distributionalHistoryFile.trim() || [distributionalBaseStateFile, `${distributionalBaseStateFile}.pending`,
      distributionalStateFile, `${distributionalStateFile}.pending`,
      env.KRAKEN_PAPER_STATE_FILE ?? "data/kraken-paper-state.json",
      configuredEnv.CONTINUOUS_RECORD_FILE ?? "data/continuous-events.jsonl.gz",
  ...
  const rollingLossFraction = positiveFractionEnv(configuredEnv.ROLLING_LOSS_FRACTION, .0075);
  const sessionLossFraction = positiveFractionEnv(configuredEnv.SESSION_LOSS_FRACTION, .0075);
  const maximumGrossNotional = positiveNumberEnv(configuredEnv.MAXIMUM_GROSS_NOTIONAL, 5_000);
  const maximumClusterPositions = integerEnv(configuredEnv.MAXIMUM_CLUSTER_POSITIONS, 1, 1, files.base.symbols.length);
  const databaseEnabled = parseBoolean(configuredEnv.DATABASE_ENABLED, true);
  const databaseRequired = parseBoolean(configuredEnv.DATABASE_REQUIRED, false);
  const databaseFlushIntervalMs = integerEnv(configuredEnv.DATABASE_FLUSH_INTERVAL_MS, 250, 25, 60_000);
  const databaseMaximumWriteLagMs = integerEnv(configuredEnv.DATABASE_MAXIMUM_WRITE_LAG_MS, 5_000, 1_000, 60_000);
  if (databaseFlushIntervalMs >= databaseMaximumWriteLagMs) throw new Error("DATABASE_FLUSH_INTERVAL_MUST_BE_BELOW_WRITE_LAG_LIMIT");
  if (databaseRequired && !databaseEnabled) throw new Error("DATABASE_REQUIRED_REQUIRES_DATABASE_ENABLED");
  for (const value of Object.values(symbolConfigs)) {
    if (value.maximumNotional > maximumGrossNotional) throw new Error(`SYMBOL_NOTIONAL_EXCEEDS_PORTFOLIO_LIMIT:${value.symbol}`);
    if (value.sizing.baseRiskFraction > Math.min(rollingLossFraction, sessionLossFraction)
      || Math.max(rollingLossFraction, sessionLossFraction) > value.sizing.maximumDrawdown) {
      throw new Error(`RISK_LIMITS_REQUIRE_TRADE_LE_LOSS_LE_DRAWDOWN:${value.symbol}`);
    }
  }
  return {
    ...baselineConfig,
    ...
    ...(distributionalSizingPolicy ? { distributionalSizingPolicy } : {}),
    ...
    portfolio: { maximumVariance: Number.POSITIVE_INFINITY, maximumClusterPositions, maximumGrossNotional, rollingLossBudgetFraction: rollingLossFraction },
    rollingLossFraction, sessionLossFraction,
    ...
    databaseEnabled, databaseRequired,
    databaseUrl: env.DATABASE_URL ?? buildDatabaseUrl(env),
    databaseFlushIntervalMs,
    ...
    databaseMaximumWriteLagMs,
    databaseStatementTimeoutMs: integerEnv(configuredEnv.DATABASE_STATEMENT_TIMEOUT_MS, 5_000, 1_000, 60_000),
    recall: {
...
function loadSymbolConfig(symbol: string, env: NodeJS.ProcessEnv, mode: TradingMode): SymbolConfig {
  ...
  return {
    symbol,
    maximumNotional: paperEntryExercise ? Math.min(25, positiveNumberEnv(env.MAXIMUM_NOTIONAL, 1_000)) : positiveNumberEnv(env.MAXIMUM_NOTIONAL, 1_000),
    ...
    sizing: { baseRiskFraction: positiveFractionEnv(env.BASE_RISK_FRACTION, .001),
      maximumDrawdown: positiveFractionEnv(env.MAXIMUM_DRAWDOWN_FRACTION, .05),
      maximumBookParticipation: positiveFractionEnv(env.MAXIMUM_BOOK_PARTICIPATION, .01),
      fractionalKelly: positiveFractionEnv(env.FRACTIONAL_KELLY, .1),
      maximumKellyFraction: positiveFractionEnv(env.MAXIMUM_KELLY_FRACTION, .05),
      targetSigmaHBps: positiveNumberEnv(env.TARGET_SIGMA_H_BPS, 20),
      minimumQualityScale: positiveFractionEnv(env.MINIMUM_QUALITY_SCALE, .1) },
    position, planner,
  };
}
...
function parseBoolean(value: string | undefined, fallback: boolean): boolean {
  ...
  throw new Error("Invalid boolean configuration; expected true or false");
}
function numberEnv(value: string | undefined, fallback: number): number { const n = value === undefined ? fallback : Number(value); if (value?.trim() === "" || !Number.isFinite(n) || n < 0) throw new Error("Invalid numeric configuration; expected a finite non-negative number"); return n; }
function positiveNumberEnv(value: string | undefined, fallback: number): number { const n = numberEnv(value, fallback); if (!(n > 0)) throw new Error("Invalid positive configuration"); return n; }
function positiveFractionEnv(value: string | undefined, fallback: number): number { const n = positiveNumberEnv(value, fallback); if (n > 1) throw new Error("Invalid positive fractional configuration"); return n; }
...
function loadDeterministicSignalConfig(env: NodeJS.ProcessEnv, mode: SignalMode, configurationVersion: string,
  ...
  return {
    ...
    pullbackRecovery: {
      ...
      maximumReversalAgeMs: numberEnv(env.RULE_PULLBACK_MAX_REVERSAL_AGE_MS,
        DEFAULT_DETERMINISTIC_SIGNAL_CONFIG.pullbackRecovery.maximumReversalAgeMs),
