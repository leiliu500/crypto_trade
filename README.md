...
Run `npm run optimize:report` against the configured PostgreSQL database to audit maker-fill calibration, the causal maker-versus-IOC entry-route shadow, and a read-only 10-minute unproductive-position exit counterfactual against the active 15-minute exit. Use `npm run optimize:report:production` in the compiled production container. The route shadow simulates displayed queue ahead from observed contra-side trades and marks each policy at its own executable exit quantity at 1, 5, 30, 60, and 300 seconds; an unfilled maker scores zero and a partial fill is weighted by its fill fraction. Delayed marks beyond one second are excluded rather than relabeled as their target horizon, and results are also sliced by symbol, side, and entry family. The report excludes records from runs with dropped telemetry and does not authorize a parameter change until the same minimum tuning duration and effective-sample requirements pass. Maker-fill deployment additionally requires ROC AUC of at least 0.55, the timeout shadow requires a positive lower 95% confidence bound for its P&L improvement, and route deployment requires both positive mean taker net return and a positive lower 95% confidence bound for taker minus maker at the configured decision horizon. Live continuation IOC activation remains code-level disabled regardless of the report.
