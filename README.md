...
- Entry: every health, dynamic-liquidity, venue, exposure, edge, cost, sizing, execution-plan, and portfolio gate must then pass. The final plan must also have positive order-level expected value and a conservative net-edge/maximum-loss ratio of at least `0.25`. Candidate sensitivity never bypasses order economics.
...
DETERMINISTIC_CONFIG_VERSION=btc-eth-profitability-controls-v6.0.0
...
Run `npm run optimize:report` against the configured PostgreSQL database to audit maker-fill calibration and a read-only 10-minute unproductive-position exit counterfactual against the active 15-minute exit. Use `npm run optimize:report:production` in the compiled production container. The report excludes orders from runs with dropped telemetry and does not authorize a parameter change until the same minimum tuning duration and effective-sample requirements pass. Maker-fill deployment additionally requires ROC AUC of at least 0.55, while the timeout shadow requires a positive lower 95% confidence bound for its P&L improvement.
