...
For repeated strategy-pipeline exercise in a paper account, `PAPER_ENTRY_EXERCISE=true` is an explicit non-economic mode. It retains signal, liquidity, exposure, sizing, portfolio, and order-lifecycle controls, but uses zero simulated fees, latency reserve, adverse-selection reserve, positive cost error, and analytical signal-uncertainty reserve. It assumes full sigma/breakout capture, reduces the cost safety factor to `1`, the minimum net edge to `0`, requires IOC/taker entry planning, adds a 5 bps IOC limit-price protection buffer against quote movement during submission, and caps each symbol at $25 notional. Startup and the dashboard label the run `EXERCISE`. The switch fails closed outside the Alpaca paper endpoint. Results from this mode are operational tests only and must never be interpreted as achievable performance.
